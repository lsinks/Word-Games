...
#  mutate(scores = map_dbl(word_name, Scoring_Word, freqs = char_frequencies, verbose = TRUE, debug_detail = TRUE)) 
word_scores <- word_scores %>% 
  mutate (score = map_dbl(word_name, Scoring_Word)) %>%
  mutate (score_guess1 = map_dbl(word_name, Scoring_Word_Unique))

#%>%
#  mutate(scores_guess1 = map_dbl(word_name, Scoring_Word_Unique, freqs = char_frequencies, verbose = TRUE, debug_detail = TRUE))
# 
# 
